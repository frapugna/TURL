...
def CF_build_input1(pgEnt, pgTitle, secTitle, caption, headers, core_entities, core_entities_text, entity_cand, config):
    ...
    try:
        ...
        tokenized_headers = [config.tokenizer.encode(str(header), max_length=config.max_header_length, add_special_tokens=False) for header in headers] ####ADDED
    input_tok = []
    input_tok_pos = []
    input_tok_type = []
    ...
    for header_i in header_span[1:]:    ####CRITICAL
...
def main():
    ...
    #data_dir = 'data/'
    data_dir = 'datasets/turl_datasets/'
    ...
    try:
        ...
        checkpoint = torch.load('models/turl_pretrained.bin', map_location=device)  #ADDED
